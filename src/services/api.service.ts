...
const fetchData =
    async (endpoint: string, params: Record<string, any> = {}): Promise<any> => {
    try {
...
export const loginApi =
    async (username: string, password: string): Promise<IUserToken> => {
...
export const getRecipeByIdApi =
    async (id: number): Promise<IRecipes> => {
    return fetchData(`/recipes/${id}`);
};

export const getRecipesByTagApi =
    async (tag: string): Promise<{ recipes: IRecipes[], total: number }> => {
    const data = await fetchData(`/recipes/tag/${tag}`);
    return {recipes: data.recipes, total: data.total};
};

export const fetchUserData =
    async (accessToken: string): Promise<IUser> => {
